...
const   Order = ({order , user}) => {
    ...
    <tr className="bg-white border-b-4 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <td className="py-4 lg:px-6 px-3 border-l-4">
           {
            (price && !order?.pay) && <button className='btn bg-red-600 py-2 px-5 lg:text-lg text-base font-semibold text-white mx-2' onClick={handleDelete}>
                    Delete
            </button>
           
           }
           {
            (price && !order?.pay) && <Link to={`/dashboard/payment/${_id}`}>
